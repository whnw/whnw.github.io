...
**不指定继承方式，对class默认按private，对struct按public**
...
**面向过程**就是分析出解决问题所需要的**步骤**，然后用函数把这些步骤一步一步实现，使用的时候一个一个依次调用就可以了。

**面向对象**是把构成问题事务分解成各个对象，**重点是划分职责，负责什么**，建立对象的目的不是为了完成一个步骤，而是为了描叙某个事物在整个解决问题的步骤中的行为。
...
优点：性能比面向对象高，因为类调用时需要实例化，开销比较大**虚函数表这种开销，要多次跳转**，比较消耗资源;比如单片机、嵌入式开发、 Linux/Unix等一般采用面向过程开发，性能是最重要的因素。
...
const 返回值对指针/引用有意义：保护数据不被修改。

const 返回值对值类型无意义：因为返回的是副本。  const double calculatePi(); // ❌ 多余：double 是值类型

不要用 const 返回值区分重载：编译器会拒绝。

优先返回 const 引用/指针：适用于返回内部数据，避免拷贝和修改
...
| 特性               | C 语言                          | C++                                          |
|--------------------|---------------------------------|----------------------------------------------|
| 支持的类型         | 仅 POD 类型（基本类型、结构体） | 所有类型（包括类、STL 容器）                 |
| 初始化时机         | 编译期（C99 前）或程序启动时    | 编译期或运行时（首次使用时初始化，线程安全） |
| 动态初始化         | ❌ 不允许（如 rand()）          | ✅ 允许                                     |
| 局部静态变量初始化 | 程序启动时（C99 前）            | 首次调用时（C++11 起线程安全）               |
| 类静态成员         | 无（C 无类）                    | 必须类外初始化（C++17 可用 inline static）   |
| 默认初始化         | 基本类型初始化为 0              | 基本类型为 0，类类型调用默认构造函数         |
...
int main() {
    ...
    std::cout << "Is \"hello\" an lvalue? " 
              << !std::is_rvalue_reference<decltype(("hello"))>::value << std::endl;
    // 输出: true
    return 0;
}
```
...
构造函数执行时，派生类对象尚未完全构造，vptr 可能指向基类的 vtable。

调用虚函数会静态绑定到当前类的版本（即基类版本）。
...
异常处理的栈展开（Stack Unwinding） 是指当异常被抛出（throw）时，程序从当前执行点开始，沿着函数调用链向上回溯，依次析构栈上的局部对象，直到找到匹配的 catch 块的过程。这一机制确保了资源的安全释放（如内存、文件句柄等），避免泄漏。
...
- **new[]和delete[]**
  ...
  如果new[]和delete一起使用，程序会认为这是一个对象占用的空间，而不是数组，因此就析构一次。同时释放的是new[]中表示长度的前4个字节的地址，应该从A-4开始释放，如果不从头释放的话回出问题。
    ...
    Widget* widgets = new Widget[3]; // 调用3次构造函数
...
Cat* cat = dynamic_cast<Cat*>(animal); // 返回 nullptr
```
...
print(const_cast<char*>(msg)); // 安全：函数不修改数据
```
...
该对象不能被程序主动修改（const），但 其值可能被外部不可控因素改变（volatile）   硬件寄存器
...
继承体系中同名非虚函数：隐藏（Hiding）而非覆盖。如果子类定义了与父类 同名但非虚的函数，父类的同名函数会被 隐藏（而非覆盖或重载）。调用规则：通过子类对象调用时，只能访问子类的版本，父类版本需显式指定。
...
在 C++ 中，虚继承（Virtual Inheritance） 是解决多重继承中 菱形继承问题（Diamond Problem） 的关键机制。它确保在继承链中，公共基类（虚基类）的成员仅保留一份副本，避免重复继承导致的二义性和资源浪费
...
64位编译器 第一种：8,8优化了   第二种：8，8   第三种：16，32   第四种：16,16
...
```c++
class Base { int x; };
class A : virtual public Base { int a; };
class B : virtual public Base { int b; };
...
通过交换临时空向量强制释放内存（C++11 前常用，现优先用 shrink_to_fit）。

```c++
std::vector<int>(vec).swap(vec); // 将 vec 与临时副本交换
```
...
在 C++ 中，std::vector 的扩容策略（通常为 1.5 倍 或 2 倍）是工程实践中的权衡结果，旨在平衡 内存使用效率 和 性能开销
...
vector<shared_ptr<Obj>> obj_buffers;
...
int main()
{
	obj_buffers.emplace_back(make_shared<Obj>());
	obj_buffers.emplace_back(make_shared<Obj>());

	thread read1(Read);
	thread read2(Read);
	sleep(1);
	thread wThread(Write);
	wThread.join();
	read1.join();
	read2.join();
