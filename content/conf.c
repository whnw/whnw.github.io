...
static void check_conf(struct menu *menu)
{
	...
	if (sym && !sym_has_value(sym)) {
		bool changable = sym_is_changable(sym);
		bool active_choice = sym_is_choice(sym) &&
			sym_get_tristate_value(sym) == yes;

		if (changable || active_choice) {
			const char *name = sym->name ? sym->name : "<anonymous choice>";
			const char *prompt = menu_get_prompt(menu);

			if (!conf_cnt++)
				printf(_("*\n* Restart config...\n*\n"));
			printf(_("* Reason: %s%s is visible but has no user value\n"),
			       sym->name ? "CONFIG_" : "", name);
			printf(_("*   prompt: %s\n"), prompt ? prompt : "<none>");
			printf(_("*   type: %s; restart condition: %s\n"),
			       sym_type_name(sym_get_type(sym)),
			       changable ? "symbol is changeable" :
				"enabled choice has no selected value");
